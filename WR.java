...
public class WR {
    ...
    static ArrayList<Player> wrs = new ArrayList<>();
    ...
    public WR() {
        try {
            File file = new File("C:\\Users\\ajthe\\IdeaProjects\\fantasyFootball\\" +
                    "src\\ff\\FantasyPros_Fantasy_Football_Statistics_WR.csv");
    ...
    public void mapPlayers() {
        for (int i = 0; i < rank.size(); i++) {
            Player q = new Player(name.get(i), points.get(i), "WR");
            wrs.add(q);
        }
    }
    ...
    public void printPlayers() {
        for (int i = 0; i < wrs.size(); i++) {
            System.out.println(wrs.get(i).getName() + " - " + wrs.get(i).getPoints());
        }
    }
    ...
    public static Player[] getTop30() {
        ...
        for (int i = 0; i < 30; i++) {
            int biggest = 0;
            int bigIndex = 0;
            Player bigPlayer = wrs.get(i);
            for (int j = 0; j < rank.size() - 30; j++) {
                if (biggest < wrs.get(j).getPoints()) {
                    biggest = wrs.get(j).getPoints();
                    bigPlayer = wrs.get(j);
                    bigIndex = j;
                }
            }
